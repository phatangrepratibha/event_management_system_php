...
include "navbar.php";
	if ($_SERVER["REQUEST_METHOD"] == "POST") 
	{
		...
		$sql = "INSERT INTO `product`(`Day`, `Date`,`Hour`, `Payment`, `Theme`, `Name`, `Phone`, `Person`,`email`,`latitude`,`longitude`) VALUES ('$day','$date', '$hour','$payment', '$theme','$name', '$phone','$person','$email','$latitude','$longitude')";

		if (mysqli_query($conn, $sql)) 
        {
			echo '<script>alert("Product Launch Booking Successfully!!!");</script>';
		} 
		else 
		{
			echo '<script>alert("Product Launch Booking failed!!! Plzz try again......");window.location.href = "prod.php";</script>';
		}
	}
...
.banner{
    min-height: 100vh;
    background: linear-gradient(rgba(0, 0, 0, 0.5), rgba(0, 0, 0, 0.5)), url("assets/28.jpg") center/cover no-repeat;
    display: flex;
...
.card-img{
    background: url("assets/7.jpeg") center/cover no-repeat;
}
.banner h2{
    padding-bottom: 40px;
    margin-bottom: 20px;
